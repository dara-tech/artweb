...
with tblactive as (
    ...
    tblimain as (
        select ClinicID, DafirstVisit, "15+" as typepatients, TypeofReturn, LClinicID, 
               SiteNameold, DaBirth, timestampdiff(year, DaBirth, @datestope) as age, 
               Sex, DaHIV, OffIn 
        from tblaimain 
        where DafirstVisit <= @datestope
        union all 
        select ClinicID, DafirstVisit, "≤14" as typepatients, '' as TypeofReturn, 
               LClinicID, SiteNameold, DaBirth, timestampdiff(year, DaBirth, @datestope) as age, 
    ...
    tblart as (
        select *, timestampdiff(month, DaArt, @datestope) as nmonthART 
        from tblaart 
...
select '10.1.Eligible MMD' as ActiveART, 
       typepatients,
