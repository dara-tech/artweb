...
function AdultVisitList() {
  ...
  const loadVisits = async () => {
    try {
      ...
      const response = await api.get('/api/visits/adult', { params });
      
      console.log('API Response Data:', response.data);
      console.log('First Visit Data:', response.data.visits?.[0]);
      
      // Debug: Check gender values in the data
      if (response.data.visits && response.data.visits.length > 0) {
        console.log('Gender values in response:', response.data.visits.map(v => ({ clinicId: v.clinicId, sex: v.sex, gender: v.gender })));
      }
  ...
  const getWHOStageBadge = (stage) => {
    if (stage === -1 || stage === null) return <Badge variant="outline" className="text-xs px-2 py-1">N/A</Badge>;
    return <Badge variant="destructive" className="text-xs px-2 py-1">Stage {stage}</Badge>;
  };

  const getEligibleBadge = (eligible) => {
    if (eligible === 1) return <Badge variant="default" className="text-xs px-2 py-1">Eligible</Badge>;
    if (eligible === 0) return <Badge variant="secondary" className="text-xs px-2 py-1">Not Eligible</Badge>;
  ...
  const sortedVisits = useMemo(() => {
    ...
    return [...visits].sort((a, b) => {
      ...
      if (localSortBy === 'age' || localSortBy === 'viralLoad') {
  ...
  const TableSkeleton = () => (
    <div className="space-y-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="grid grid-cols-10 gap-2 xl:gap-4 px-4 xl:px-6 py-4">
      ...
      <div className="flex items-center justify-between mt-6 px-6 py-4 bg-slate-50 rounded-lg">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            className="h-8 px-3 text-slate-600 border-slate-200 hover:bg-slate-100"
          >
          ...
          {Array.from({ length: endPage - startPage + 1 }, (_, i) => startPage + i).map(page => (
            <Button
              ...
              className={`h-8 w-8 p-0 ${currentPage === page ? 'bg-slate-800 text-white' : 'text-slate-600 border-slate-200 hover:bg-slate-100'}`}
            >
              {page}
            </Button>
          ))}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-500 font-medium">
              Adult patient visit management • {totalVisits.toLocaleString()} total visits
            </p>
          </div>
      ...
      {/* Visits List */}
      {loading ? (
        <div className="bg-white border-0 rounded-xl shadow-sm overflow-hidden">
          {/* Desktop Table Skeleton */}
          <div className="hidden lg:block">
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 border-b">
              <div className="grid grid-cols-10 gap-2 xl:gap-4 px-4 xl:px-6 py-4 text-sm font-semibold text-gray-700">
                <div className="col-span-1">Clinic ID</div>
                <div className="col-span-1">Site</div>
                <div className="col-span-1">Date</div>
                <div className="col-span-1">ART Number</div>
                <div className="col-span-1 text-center">Age</div>
                <div className="col-span-1 text-center">Sex</div>
                <div className="col-span-1 text-center">Viral Load</div>
                <div className="col-span-1">Next Appointment</div>
            ...
            <div className="hidden lg:block">
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 border-b">
                <div className="grid grid-cols-10 gap-2 xl:gap-4 px-4 xl:px-6 py-4 text-sm font-semibold text-gray-700">
                  ...
                  <div className="col-span-1">
                    <SortableHeader field="site_code">Site</SortableHeader>
                  </div>
                  <div className="col-span-1">
                  ...
                  <div className="col-span-1 text-center">
                    <SortableHeader field="viralLoad">Viral Load</SortableHeader>
                  </div>
                  <div className="col-span-1">
              ...
              <div className="divide-y divide-slate-100">
                {sortedVisits.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-slate-500">
                    ...
                    <h3 className="text-lg font-medium mb-2 text-slate-700">No visits found</h3>
                    <p className="text-sm text-slate-400 max-w-md text-center">Try adjusting your search or filter criteria</p>
                  </div>
                ) : (
                  sortedVisits.map((visit, index) => (
                    <div
                      key={`${visit.clinicId}-${visit.visitId}-${visit.visitDate}-${index}`}
                      className="grid grid-cols-10 gap-2 xl:gap-4 px-4 xl:px-6 py-4 hover:bg-slate-50 transition-colors duration-200 group items-center"
                    >
                      <div className="col-span-1 font-mono text-sm text-slate-600 truncate">
                        {String(visit.clinicId).padStart(6, '0')}
                      </div>
                      <div className="col-span-1 text-sm text-slate-700 truncate">
                        {visit.site_code || 'N/A'}
                      </div>
                      <div className="col-span-1 text-sm text-slate-700 truncate">
                      ...
                      <div className="col-span-1 text-sm text-slate-700 text-center">
                        {visit.age ? `${visit.age}y` : 'N/A'}
                      </div>
                      <div className="col-span-1 flex justify-center">
                      ...
                      <div className="col-span-1 flex justify-center">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => navigate(`/visits/adult/${visit.clinicId}/${visit.visitId}`)}>
                              <Eye className="w-4 h-4 mr-2" />
                              View Details
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => navigate(`/visits/adult/${visit.clinicId}/${visit.visitId}/edit`)}>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex flex-wrap gap-1">
                            ...
                            {getWHOStageBadge(visit.whoStage)}
                            {getEligibleBadge(visit.eligible)}
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-sm truncate">{visit.artNumber || 'N/A'}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-gray-600">Age:</span>
                            <span className="text-slate-700">{visit.age ? `${visit.age} years` : 'N/A'}</span>
                          </div>
                          <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold">{visit.weight || 'N/A'} kg</span>
                          </div>
                          <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <Calendar className="w-4 h-4 text-gray-400 flex-shrink-0" />
                            <span className="font-medium text-gray-600">Next Apt:</span>
                            <span className="text-slate-700 truncate">{visit.nextAppointment && visit.nextAppointment !== '1900-01-01' ? formatDate(visit.nextAppointment) : 'N/A'}</span>
                          </div>
                        </div>
