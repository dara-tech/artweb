...
export const siteApi = {
  ...
  getAllSites: async () => {
    const response = await api.get('/api/site-operations/sites')
  ...
  getSiteStats: async (siteCode) => {
    if (siteCode) {
      const response = await api.get(`/api/site-operations/sites/${siteCode}/stats`)
      return response.data
    } else {
      const response = await api.get('/api/site-operations/sites')
      return response.data
  ...
  updateSite: async (siteCode, siteData) => {
    const response = await api.put(`/api/sites/${siteCode}`, siteData)
    return response.data
  },

  // Delete site
  deleteSite: async (siteCode) => {
    const response = await api.delete(`/api/sites/${siteCode}`)
    return response.data
  },
  ...
  testSiteConnection: async (siteCode) => {
    const response = await api.get(`/api/site-operations/sites/${siteCode}/test`)
    return response.data
  },

  // Execute custom query on site
  executeSiteQuery: async (siteCode, query) => {
    const response = await api.post(`/api/site-operations/sites/${siteCode}/query`, { query })
    return response.data
  }
}
