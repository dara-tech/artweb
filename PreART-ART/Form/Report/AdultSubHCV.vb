...
Public Class AdultSubHCV
    Dim Rdr As MySqlDataReader
    Dim dbtem As MySqlConnection
    Dim Sdate, Edate As Date
    Private Sub AdultSubHCV_BeforePrint(sender As Object, e As PrintEventArgs) Handles Me.BeforePrint
        ...
        'Dim CmdSearch As New MySqlCommand("SELECT count(tblavhydrug.DrugName) as Num, tblavhydrug.DrugName  FROM   patientactive LEFT OUTER JOIN  tblavhydrug ON patientactive.Vid = tblavhydrug.Vid WHERE  (tblavhydrug.Status IN (0, 2)) and patientactive.DafirstVisit <= '" & Format(Edate, "yyyy-MM-dd") & "' group by tblavhydrug.DrugName ORDER BY Num desc", Cnndb)
        ...
        Dim CmdSearch As New MySqlCommand("SELECT count(tblavhydrug.DrugName) as Num, tblavhydrug.DrugName  FROM(select ai.ClinicID,ai.Sex,ai.DafirstVisit,mv.DatVisit,mv.Vid from" &
                ...
                "where ap.Status is null) patientactive LEFT OUTER JOIN  tblavhydrug ON patientactive.Vid = tblavhydrug.Vid WHERE  (tblavhydrug.Status IN (0, 2)) and " &
                "patientactive.DafirstVisit <= '" & Format(Edate, "yyyy-MM-dd") & "' group by tblavhydrug.DrugName ORDER BY Num desc", Cnndb)
        Rdr = CmdSearch.ExecuteReader
        Dim q1 As String
        While Rdr.Read
            Mydata.Regimen.AddRegimenRow(" " & Rdr.GetValue(1).ToString.Trim, Rdr.GetValue(0).ToString)
