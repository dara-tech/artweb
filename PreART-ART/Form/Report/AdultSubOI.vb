...
Public Class AdultSubOI
    Dim Rdr As MySqlDataReader
    Dim dbtem As MySqlConnection
    Dim Sdate, Edate As Date
    Private Sub AdultSubOI_BeforePrint(sender As Object, e As PrintEventArgs) Handles Me.BeforePrint
        ...
        'Dim CmdSearch As New MySqlCommand("SELECT count(tblavoidrug.DrugName) as Num, tblavoidrug.DrugName  FROM   patientactive LEFT OUTER JOIN  tblavoidrug ON patientactive.Vid = tblavoidrug.Vid WHERE  (tblavoidrug.Status IN (0, 2)) and patientactive.DafirstVisit <= '" & Format(Edate, "yyyy-MM-dd") & "' group by tblavoidrug.DrugName ORDER BY Num desc", Cnndb)
        ...
        Dim CmdSearch As New MySqlCommand("SELECT count(d.DrugName) as Num, d.DrugName  FROM(" &
            "select * from(select patientactive.ClinicID,tblavoidrug.DrugName,tblavoidrug.Status from" &
            "(select ai.ClinicID,ai.Sex,ai.DafirstVisit,mv.DatVisit,mv.Vid from" &
            ...
            "where ap.Status is null) patientactive " &
            "LEFT OUTER JOIN  tblavoidrug ON patientactive.Vid = tblavoidrug.Vid " &
            "WHERE  (tblavoidrug.Status IN (0, 2)) and patientactive.DafirstVisit <= '" & Format(Edate, "yyyy-MM-dd") & "' ) o " &
            "union(select patientactive.ClinicID,tblavtptdrug.DrugName,tblavtptdrug.Status from" &
            "(select ai.ClinicID,ai.Sex,ai.DafirstVisit,mv.DatVisit,mv.Vid from" &
            "(select v.ClinicID,v.DatVisit,v.Vid from tblavmain v " &
            "inner join(select ClinicID,max(DatVisit) as DatVisit,Vid from tblavmain where DatVisit<='" & Format(Edate, "yyyy-MM-dd") & "' group by ClinicID ) vv " &
            "on v.ClinicID=vv.ClinicID and v.DatVisit=vv.DatVisit) mv " &
            ...
            "left join(select * from tblavpatientstatus where Da<='" & Format(Edate, "yyyy-MM-dd") & "') ap on ai.ClinicID=ap.ClinicID " &
            "where ap.Status is null) patientactive " &
            "LEFT OUTER JOIN  tblavtptdrug ON patientactive.Vid = tblavtptdrug.Vid " &
            "WHERE  (tblavtptdrug.Status IN (0, 2)) and patientactive.DafirstVisit <= '" & Format(Edate, "yyyy-MM-dd") & "')) d " &
            "group by d.DrugName ORDER BY Num desc;", Cnndb)
        Rdr = CmdSearch.ExecuteReader
        Dim q1 As String
        While Rdr.Read
            Mydata.Regimen.AddRegimenRow(" " & Rdr.GetValue(1).ToString.Trim, Rdr.GetValue(0).ToString)
