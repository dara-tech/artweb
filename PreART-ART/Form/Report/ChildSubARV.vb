...
Public Class ChildSubARV
    ...
    Private Sub AdultSubARV_BeforePrint(sender As Object, e As PrintEventArgs) Handles Me.BeforePrint
        ...
        'Dim CmdSearch As New MySqlCommand("SELECT childpatientactive.ClinicID, tblcvarvdrug.DrugName, tblcvarvdrug.Dose, tblcvarvdrug.Status ,childpatientactive.Weight FROM   childpatientactive LEFT OUTER JOIN  tblcvarvdrug ON childpatientactive.Vid = tblcvarvdrug.Vid WHERE  (tblcvarvdrug.Status IN (0, 2)) and childpatientactive.DaFirstVisit <= '" & Format(Edate, "yyyy-MM-dd") & "' ORDER BY childpatientactive.ClinicID, tblcvarvdrug.DrugName", Cnndb)
        ...
        Dim CmdSearch1 As New MySqlCommand("select * from (select c.regimen,count(c.regimen) as number_of_Reg from (" &
            "select distinct v.clinicid,v.DatVisit,v.vid,dr.regimen,v.DaApp from tblcvmain v inner join" &
            "(Select distinct clinicid,max(DatVisit) as datvisit from tblcvmain " &
            ...
            "SELECT distinct vid,group_concat(DrugName order by DrugName separator ' + ') as regimen FROM preart.tblcvarvdrug " &
            "where Status<>1 group by vid) dr on dr.vid=v.vid " &
            "left outer join(select * from tblcvpatientstatus where Da<='" & Format(Edate, "yyyy-MM-dd") & "') st on st.clinicid=v.ClinicID " &
